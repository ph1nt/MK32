...
void update_oled() {
  ...
  if (xQueueReceive(layer_recieve_q, &curr_layout, (TickType_t)0)) {
    snprintf(_oledtext, 16, "LAYER %d", curr_layout);
    ssd1306_display_text(&dev, 0, _oledtext, 16, false);
  }
  if (xQueueReceive(led_recieve_q, &current_led, (TickType_t)0)) {
    snprintf(_oledtext, 16, "%s %s %s", "NUM", "CAPS", "SCROLL");
    ssd1306_display_text(&dev, 2, _oledtext, 16, false);
  }
  snprintf(_oledtext, 16, "Battery %d%%", get_battery_level());
  ssd1306_display_text(&dev, 3, _oledtext, 16, false);
}

// shut down OLED
void deinit_oled(void) { ssd1306_clear_screen(&dev, false); }
