...
/*
 * Current state of the keymap, each cell will hold the location of the key in
 * the key report, if a key is not in the report it will be set to 0
 */
...
uint8_t *check_key_state(uint16_t **keymap) {
  ...
  for (uint8_t col = 0; col < MATRIX_COLS; col++) {
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
      // if there are no changes on this matrix position, skip to next
      // position
      if (MATRIX_STATE[row][col] == matrix_prev_state[row][col]) continue;
      uint16_t report_index = (2 + col + row * KEYMAP_COLS);
      keycode = keymap[row][col];
      ESP_LOGI("KEY", "row %d, col %d, keycode %d", row, col, keycode);
      // checking if the keycode is transparent
      // TODO check it should be recurent
      if (keycode == KC_TRNS) {
        if (current_layout == 0) {
      ...
      if (MATRIX_STATE[row][col] == 0) {
        ...
        if ((layouts[prev_layout][row][col] >= LAYER_HOLD_BASE_VAL) &&
            (keycode <= LAYER_HOLD_MAX_VAL) && (layer_hold_flag == 1)) {
          current_layout = 0;
          layer_hold_flag = 0;
#ifdef OLED_ENABLE
          xQueueSend(layer_recieve_q, &current_layout, (TickType_t)0);
