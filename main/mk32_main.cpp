...
=======
  ble_connected_oled();
  bool CON_LOG_FLAG = false; // Just because I don't want it to keep logging the
                             // same thing a billion times
  while (1) {
    if (halBLEIsConnected() == 0) {
      if (CON_LOG_FLAG == false) {
...
#ifdef SLAVE
  xTaskCreatePinnedToCore(slave_scan, "Scan matrix changes for slave", 4096,
                          xKeyreportTask, configMAX_PRIORITIES, NULL, 1);
#ifdef R_ENCODER_SLAVE
  xTaskCreatePinnedToCore(slave_encoder_report,
                          "Scan encoder changes for slave", 4096, NULL,
                          configMAX_PRIORITIES, NULL, 1);
#endif
  espnow_send();
#endif

  // If the device is a master for split board initialize receiving reports from
  // slave
#ifdef SPLIT_MASTER
...
#ifdef BATT_STAT
  init_batt_monitor();
  xTaskCreatePinnedToCore(battery_reports, "battery reports", 4096, NULL,
                          configMAX_PRIORITIES, NULL, 1);
